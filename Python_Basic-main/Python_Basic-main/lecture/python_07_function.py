...
# Default Rarameter
#  - 만약에 함수 호출 시 입력해야하는 parameter가 empty인 경우
#    Default Parameter로 대체해주는 기능!
#  - 끝에서부터 채워야함
# ex) def test(a=1, b=5, c=2):   (O)
# ex) def test(a, b=5, c=2):   (O)
# ex) def test(a, b, c=2):   (O)
...
#  - return이 없는 경우 들여쓰기 종료하면 함수 종료로 간주
...
#  - 인자(parameter): 함수 입력값
#  - 함수 입력값의 갯수가 함수를 호출할 때 마다 매번 다른 경우
#    -> 가변 길이 인자를 사용해야함!!!
